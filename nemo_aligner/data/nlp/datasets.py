...
class MathDataset(RewardModelDataset):
    """This class assumes each line of the dataset file is a dictionary with "problem" and "expected_answer" field,
    where "problem" is a string representing the input prompt, and "expected_answer" is a number or string (will be converted to string).
    Note that when training the model with multiple datasets which contain different attributes,
    we should set missing attributes to model.regression.loss_mask_val(according to training_rm.yaml)
    in the dataset files so that their losses are masked. At least one attribute should be present for each sample.
    ...
    def __init__(
        self, cfg, tokenizer, name, data_prefix, documents, data, seq_length, seed, drop_last=True, apply_chat=True,
    ):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_json:
            # `sample` is a regular Python list.
            sample_tensor = torch.tensor(sample_text, dtype=torch.int64)
        else:
            # `sample` is a NumPy array.
        ...
        output = {
            "problem": sample_tensor,
            "length": text_np.shape[0],
            # "position_ids": position_ids,
            # "attention_mask": attention_mask,
            # "loss_mask": loss_mask,
